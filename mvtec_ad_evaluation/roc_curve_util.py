...
def main2():
    ...
    pred = cv2.imread('C:\\Users\\IntekPlus\\Desktop\\vscode\\MVTec_AD\\dataset\\mvtec_anomaly_detection\\capsule\\ground_truth\\crack\\000_mask.png', cv2.IMREAD_GRAYSCALE)
    pred = pred / 255.0
    anomaly_maps.append(pred)
    
    pred = cv2.imread('000_mask_1.png', cv2.IMREAD_GRAYSCALE)
    pred = pred / 255.0
    anomaly_maps.append(pred)
    
    pred = cv2.imread('000_mask_2.png', cv2.IMREAD_GRAYSCALE)
    pred = pred / 255.0
    anomaly_maps.append(pred)
    
    pred = cv2.imread('000_mask_3.png', cv2.IMREAD_GRAYSCALE)
    pred = pred / 255.0
    anomaly_maps.append(pred)

    anomaly_maps.append(np.zeros_like(pred))
    anomaly_maps.append(np.ones_like(pred))
